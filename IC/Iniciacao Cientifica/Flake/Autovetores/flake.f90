PROGRAM	flake
     IMPLICIT NONE
     INTEGER::r, n, k, i, a, j, m, ac, b, iaq, jaq, INFO, LWORK, l, t, ja, ia
     DOUBLE PRECISION,ALLOCATABLE:: AQ(:,:),x(:),y(:),z(:), WORK(:), W(:), Energy(:), q(:)
     DOUBLE PRECISION:: pi, rad, acc, a1x, a1y, p, a2x, a2y, d, x1 ,y1, z1, gamma,e_site, Ef, Ef1, Ef2, GAP, c
...
!!!!!!!!! diagonalização da matriz hamiltoniana obtida com os cálculos acima para encontrar os autovalores da estrutura  !!!!!!!!!
             ...
             WRITE(filename,'(I2)')k
              IF(k.lt.10) filename='0'//TRIM(ADJUSTL(filename))
              filename='autovetor-'//TRIM(ADJUSTL(filename))//'.atv'    
             OPEN(UNIT=5,FILE=TRIM(filename))

         ALLOCATE(WORK(1))
         LWORK=-1
         CALL DSYEV('V','L',a,AQ,a,W,WORK,LWORK,INFO)
         LWORK=WORK(1)
         DEALLOCATE(WORK)
         ALLOCATE(WORK(LWORK))
         CALL DSYEV('V','L',a,AQ,a,W,WORK,LWORK,INFO)
